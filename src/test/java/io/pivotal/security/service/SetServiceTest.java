...
public class SetServiceTest {
  ...
  public void setUp() throws Exception {
    ...
    currentUserPermissions = new AccessControlEntry(userContext.getAclUser(),  Arrays.asList(READ, WRITE, DELETE, WRITE_ACL, READ_ACL));
  ...
  public void performSet_whenThereIsAnExistingCredentialAndOverwriteIsTrue_itLogsCREDENTIAL_UPDATE() {
    when(credentialDataService.findMostRecent(CREDENTIAL_NAME)).thenReturn(existingCredential);
    when(credentialDataService.save(any(Credential.class))).thenReturn(new PasswordCredential().setEncryptor(encryptor));
  ...
  public void performSet_whenThereIsAnExistingCredential_itShouldCallVerifyCredentialWritePermission() {
    ...
    verify(permissionService).verifyCredentialWritePermission(userContext, existingCredential.getCredentialName());
  }
  ...
  public void performSet_whenThereIsNoExistingCredential_itShouldNotCallVerifyCredentialWritePermission() {
    ...
    verify(permissionService, times(0)).verifyCredentialWritePermission(userContext, existingCredential.getCredentialName());
  }
