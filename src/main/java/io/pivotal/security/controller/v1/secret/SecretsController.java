...
public class SecretsController {
  ...
  private ResponseEntity handleRegenerateRequest(HttpServletRequest request,
    ...
    // objectMapper.readValue(requestString, SecretRegenerateRequest.class);
    // requestBody.validate();
    requestInputStream.reset();
    DocumentContext parsedRequestBody = jsonContextFactory.getObject().parse(requestInputStream);
    return retryingAuditedStoreSecret(request, authentication, namedSecretGenerateHandler,
  ...
  public ResponseError handleInputNotReadableException(Exception exception) throws Exception {
    ...
    } else if (cause instanceof JsonMappingException && cause.getMessage()
      ...
      try {
        ...
        if (shouldWriteNewEntity) {
          NamedSecret newEntity = requestBody
              .createNewVersion(existingNamedSecret, encryptor, applicationContext);
